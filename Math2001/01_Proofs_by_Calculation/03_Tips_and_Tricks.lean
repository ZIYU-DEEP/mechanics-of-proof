...
-- Example 1.3.2
-- Creating the hypothesis in the right-hand side
...
-- Example 1.3.3
-- Creating one side of the hypothesis inside a goal
...
-- Example 1.3.4
-- Creating one side of the hypothesis by addition/substraction/multiplication/division
...
-- Example 1.3.5
-- Creating one side of the hypothesis
...
example {a b : ℚ} (h1 : a - 3 = 2 * b) : a ^ 2 - a + 3 = 4 * b ^ 2 + 10 * b + 9 :=
  calc
    ...
    _ = (a - 3) ^ 2 + 5 * (a - 3) + 15 - 6 := by ring
    _ = (2 * b) ^ 2 + 5 * (2 * b) + 15 - 6 := by rw [h1]
...
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    ...
    _ = (z ^ 2) * (z ^ 2 - 2) - z * (z ^ 2 - 2) + (z ^ 2 - 2) + 3 := by ring
